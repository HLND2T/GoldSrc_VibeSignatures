...
class GitHistoryTests(unittest.TestCase):
    def test_explicit_source_must_match_checkout(self):
        with self.assertRaises(release_git.QueryError):
            release_git.GitHistory(self.root, 10000, "f" * 40)
        history = release_git.GitHistory(self.root, 10000, self.git("rev-parse", "HEAD"))
        self.assertIn("fix source evidence", history.query({"command": "log"}))
    ...
    def test_write_commands_option_injection_and_path_escape_are_rejected(self):
        before = self.snapshot()
        cases = [
            {"command": command} for command in ("reset", "checkout", "clean", "commit", "config", "fetch", "push")
        ]
        cases += [
            {"command": "log", "args": ["--output=owned"]},
    ...
    def test_thirdparty_generated_and_binary_bodies_are_not_returned(self):
        for relative in (
            "thirdparty/vendor.cpp",
            "nested/thirdparty/vendor.cpp",
            "Build/generated.cpp",
            "nested/code.g.cs",
        ):
    ...
    def test_git_has_no_credentials_and_cannot_invoke_external_diff(self):
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "RELEASE_NOTES_API_KEY": "api-secret",
                "GIT_EXTERNAL_DIFF": "not-a-program",
                "GIT_CONFIG_COUNT": "1",
            },
        ):
    ...
    def test_git_query_timeout_fails_closed(self):
        with (
            patch.object(release_git.subprocess, "run", side_effect=subprocess.TimeoutExpired("git", 30)),
            self.assertRaises(release_git.QueryError),
        ):
            self.history.query({"command": "log"})
