    ...
    def tool_call(self, payload, provider):
        if self.native_write:
            ...
            if self.native_write == "Edit":
                return "Edit", {
                    "file_path": str(self.repository / "source.cpp"),
                    "old_string": "cli-history-source-marker",
                    "new_string": "modified",
                }
        ...
        if self.round >= 3:
            return None
        name = next(
            (tool.get("name") for tool in payload.get("tools", []) if "git_history" in tool.get("name", "")),
            "missing_git_tool",
        )
    ...
    def responses_events(self, payload):
        ...
        return [
            ...
            (
                ...
                {"item_id": "msg_test", "output_index": 0, "content_index": 0, "delta": NOTES},
            ),
            (
                "response.output_text.done",
                {"item_id": "msg_test", "output_index": 0, "content_index": 0, "text": NOTES},
            ),
            (
                "response.content_part.done",
                {"item_id": "msg_test", "output_index": 0, "content_index": 0, "part": item["content"][0]},
            ),
            ("response.output_item.done", {"output_index": 0, "item": item}),
            ("response.completed", {"response": response}),
        ]
        ...
        if tool:
            return [
                ...
                (
                    ...
                    {
                        "index": 0,
                        "content_block": {"type": "tool_use", "id": f"tool_{self.round}", "name": tool[0], "input": {}},
                    },
                ),
                (
                    "content_block_delta",
                    {"index": 0, "delta": {"type": "input_json_delta", "partial_json": json.dumps(tool[1])}},
                ),
                ("content_block_stop", {"index": 0}),
                (
                    "message_delta",
                    {"delta": {"stop_reason": "tool_use", "stop_sequence": None}, "usage": {"output_tokens": 20}},
                ),
                ("message_stop", {}),
            ]
        return [
            ...
            (
                "message_delta",
                {"delta": {"stop_reason": "end_turn", "stop_sequence": None}, "usage": {"output_tokens": 20}},
            ),
            ("message_stop", {}),
        ]
    ...
    def test_pinned_clis_query_git_and_reject_git_writes(self):
        ...
        for provider in ("codex", "claude"):
            with self.subTest(provider=provider):
                ...
                self.assertEqual(
                    NOTES,
                    release.run_cli_once(
                        "Investigate Git history and generate bilingual notes.",
                        provider,
                        "release-test-model",
                        self.endpoint,
                        "fake-test-key",
