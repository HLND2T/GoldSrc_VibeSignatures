...
The production workflow downloads schema-6 snapshot and metadata assets from the immutable published GitHub Release selected by the release event. Local and CI builds use a generated minimal fixture unless `GSVIBE_GAMESYMBOLS_DIR` points to an explicit downloaded asset directory. The build emits index schema v4. Each binary entry in a version JSON carries raw-byte MD5, SHA-256, CRC-32/ISO-HDLC, CRC-64/XZ, and byte size. Every version entry carries the SHA-256 and byte size of the exact UTF-8 JSON response body, and its URL is strictly `<gameVersion>.<sha256>.json`. The browser verifies those bytes before decoding or parsing the snapshot.
