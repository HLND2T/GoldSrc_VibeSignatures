"""Bounded Git history MCP; adapted from MetaHookSv commit 061d0e9a9a1a7d3c4e75827ce1717a00842c6f81."""
...
TOOL = {
    ...
    "inputSchema": {
        ...
        "properties": {
            ...
            "base": {
                "type": "string",
                "description": "Required for diff; optional for log to list base..revision; ancestor commit/tag",
            },
            "path": {
                "type": "string",
                "description": "Optional literal repository-relative path; show reads this file at revision",
            },
...
class GitHistory:
    def __init__(self, repository, budget, source_sha=None):
        ...
        if source_sha is not None and (not re.fullmatch(r"[0-9a-f]{40}", source_sha) or source_sha != self.head):
            raise QueryError("Checkout does not match SOURCE_SHA")
    ...
    def query(self, arguments):
        ...
        if path and (
            set(path.lower().split("/")) & EXCLUDED_DIRECTORIES
        ...
        if command == "log":
            ...
            query = (
                ["show", "--no-show-signature", "--no-ext-diff", "--no-textconv", f"{revision}:{path}"]
                if path
                else [
                    "show",
                    "--no-show-signature",
...
def serve(repository, budget, source_sha=None):
    history = GitHistory(repository, budget, source_sha)
    while True:
...
if __name__ == "__main__":
    ...
    serve(options.repository, options.budget, options.source_sha)
