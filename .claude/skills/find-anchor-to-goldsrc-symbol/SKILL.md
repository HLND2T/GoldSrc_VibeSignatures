...
- **Numeric scalar (`scalar`)**: a verified uint32 value, such as a frame-ring element's byte stride. Anchor its owning function, independently verify current-binary dataflow, and map the pseudocode through `found_scalar`. Emit only `scalar_name` and `scalar_value`; do not require a specific instruction or runtime signature. Consumers directly use the value for the matching binary identity. Equivalent IMUL and LEA/SHL/ADD/SUB forms are valid evidence when verified; unknown or conflicting dataflow fails closed. Never copy a reference build's value or mistake typed-element coefficients for byte sizes.
