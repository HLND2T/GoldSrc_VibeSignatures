...
description: |
  ...
  Use this skill when you need a signature for an instruction like call dword ptr [eax+538h], where the vfunc offset (0x538) must be explicitly fixed in the first instruction bytes. GoldSrc x86 only: vfunc slots are 4 bytes.
  Triggers: vfunc offset signature, signature for vfunc offset, virtual function offset signature, call [reg+offset] signature
---

# Generate Signature for VFunc Offset (GoldSrc)

Generate a unique hex byte signature that locates an instruction containing a specific vfunc offset (for example: `call dword ptr [eax+538h]`). Applies to GoldSrc **PE32/I386** (Windows) and **ELF32/I386** (Linux) binaries only.
...
**GoldSrc x86 note:** an indirect virtual call is `call dword ptr [reg + disp]` (e.g. `FF 90 38 05 00 00`).
The displacement `disp` is the `vfunc_offset`; since slots are 4 bytes, `vfunc_index = disp / 4`. This script locks
the displacement bytes into the signature so the offset is recoverable at runtime.
...
def main():
    ...
    target_vfunc_offset = <vfunc_offset>   # e.g. 0x538 from "call dword ptr [eax+538h]"
    min_sig_bytes = 6
    max_sig_bytes = 96
    max_instructions = 64
...
```yaml
vfunc_sig: "FF 90 38 05 00 00 8B 4D ?? 85 C9 74 ??"
vfunc_sig_va: 0x10244000
vfunc_sig_disp: 0
vfunc_inst_length: 6
vfunc_disp_offset: 2
vfunc_disp_size: 4
...
On GoldSrc x86, `vfunc_index = vfunc_offset / 4` (4-byte slots). For `0x538` → `vfunc_index = 0x14E`.
