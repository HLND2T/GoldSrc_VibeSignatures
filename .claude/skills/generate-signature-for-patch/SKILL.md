...
# Generate Signature for Patch (GoldSrc)

Generate a unique hex byte signature that locates an instruction to be patched at runtime, along with the replacement `patch_bytes`. Applies to GoldSrc **PE32/I386** (Windows) and **ELF32/I386** (Linux) binaries only.
...
**GoldSrc x86 patch patterns** (instruction widths differ from x86-64 Source2):

- **Skip conditional branch — near `0F 8x rel32` (6 bytes):** Replace with `E9 <new_rel32> 90`. Compute
  `new_rel32 = original_branch_target - (patch_addr + 5)`.
- **Skip conditional branch — short `7x rel8` (2 bytes):** Replace `7x rel8` with `EB rel8` (same 2-byte length).
- **Force conditional branch to fall through:** NOP the entire jcc instruction (`90` per byte).
- **NOP a `call rel32` (`E8 xx xx xx xx`, 5 bytes):** Replace with `90 90 90 90 90`.
- **NOP a `call dword ptr [reg+disp]` (`FF 90 xx xx xx xx`, 6 bytes):** Replace all 6 bytes with `90`.
- **Change immediate operand:** Modify the immediate bytes in-place.

**Length rule:** the replacement must have the **same byte length** as the original instruction. `E9 rel32` is 5
bytes, so a 6-byte near jcc becomes `E9 <new_rel32> 90` (jmp + one NOP). A short `7x rel8` (2 bytes) becomes
`EB rel8` (2 bytes, no padding needed).
...
### Example Output (GoldSrc x86)

Patch effect: skip conditional branch `jbe` → unconditional `jmp` (the `if` block becomes dead code). On x86 a
near `jbe` is `0F 86 rel32` (6 bytes).
...
In this example:
- Original instruction: `jbe loc_10200EE4` (6 bytes: `0F 86 AF 00 00 00`)
- Patch converts it to `jmp loc_10200EE4` + `nop`: `E9 B0 00 00 00 90`
- `new_rel32` = `0x10200EE4 − (0x10200E2F + 5)` = `0xB0` → `B0 00 00 00`
- The branch now always jumps, making the if-block dead code
- Note the trailing `90` padding: `E9 rel32` is 5 bytes, and the original near jcc is 6 bytes
