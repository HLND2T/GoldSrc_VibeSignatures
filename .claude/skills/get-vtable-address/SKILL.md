...
description: |
  Find a function's vtable address using IDA Pro MCP. Use this skill when you want to get the exact virtual address of a class. GoldSrc x86 only: 4-byte vtable slots, Linux Itanium ABI skips 8 bytes of metadata.
  Triggers: get vftable address, get virtual function table, find vtable, get vtable
---

# Get VTable address and size (GoldSrc)

Find a class's virtual function table by class name. Get its address and size in a single step. Applies to GoldSrc **PE32/I386** (Windows) and **ELF32/I386** (Linux) binaries only.
...
Run this single Python script using `mcp__ida-pro-mcp__py_eval`, replacing `<CLASS_NAME>` with the actual class name (e.g., `CBasePlayer`, `CBaseAnimating`, `CHud`):
...
# Linux: _ZTV<len>ClassName  (e.g. _ZTV10CBasePlayer)
# GoldSrc x86 Itanium ABI: first vfunc pointer is at +0x8 (2 x 4-byte pointers).
if vtable_start is None:
    linux_name = f"_ZTV{len(class_name)}{class_name}"
    addr = ida_name.get_name_ea(idaapi.BADADDR, linux_name)
    if addr != idaapi.BADADDR:
        vtable_start = addr + 0x8  # skip offset-to-top + typeinfo ptr (4 bytes each)
        vtable_symbol = f"{linux_name} + 0x8"
        is_linux = True
        method = "direct"
...
for i in range(1000):
    ...
    ptr_value = ida_bytes.get_dword(ea)
...
**Platform-specific vfunc counting (GoldSrc x86)**:
...
- **vtable_symbol**: The IDA symbol. e.g. `??_7CBasePlayer@@6B@` or `_ZTV10CBasePlayer + 0x8`
...
On GoldSrc x86 the vtable slot width is always **4 bytes**. The Linux `_ZTV` metadata is 8 bytes
(offset-to-top + typeinfo, 4 bytes each), so the first vfunc pointer is at `+0x8` — **not** `+0x10` (x86-64 Source2).
