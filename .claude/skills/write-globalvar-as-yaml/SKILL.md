...
description: Write global variable analysis results to the exact analyzer-bound YAML artifact path using IDA Pro MCP. Use this skill after completing global variable identification and signature generation.
---

# Write Global Variable IDA Analysis Output as YAML (GoldSrc)

Persist global variable analysis results to the exact output path in the invocation prompt's artifact contract. Match the
`<gv_name>.<platform>.yaml` basename and never derive the YAML path from the binary. Applies to GoldSrc **PE32/I386**
