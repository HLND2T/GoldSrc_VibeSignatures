...
description: |
  ...
  Use this skill when you need a signature for an instruction like mov [ecx+1A8h], eax or cmp dword ptr [eax+0B0h], 0, where the struct offset must be explicitly fixed in the first instruction bytes.
...
# Generate Signature for Struct Offset (GoldSrc)

Generate a unique hex byte signature that locates an instruction containing a specific struct member offset (for example: `mov [ecx+1A8h], eax`, `cmp dword ptr [eax+0B0h], 0`). Applies to GoldSrc **PE32/I386** (Windows) and **ELF32/I386** (Linux) binaries only.
...
def main():
    ...
    target_struct_offset = <struct_offset>   # e.g. 0x1A8 from "mov [ecx+1A8h], eax"
    min_sig_bytes = 6
    max_sig_bytes = 96
    max_instructions = 64
...
```yaml
struct_sig: "C7 81 A8 01 00 00 01 00 00 00 8B 4D ?? 85 C9 74 ??"
struct_sig_va: 0x10244000
offset_sig_disp: 0
