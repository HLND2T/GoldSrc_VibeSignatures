"""Domain models and backend abstractions for process reporting."""
...
class NullProcessReporter:
    """Default reporter that preserves local CLI behavior without emitting events."""
...
class ConsoleProcessReporter:
    """Write the current process event contract as JSONL for local diagnostics."""

    def __init__(self, stream: TextIO | None = None) -> None:
        self._stream = stream or sys.stdout

    def _write(self, event: ProcessEvent) -> None:
        print(json.dumps(event.to_dict(), ensure_ascii=False, sort_keys=True), file=self._stream)

    def initialize_run(self, plan: dict[str, Any], run_id: str | None = None) -> str:
        initialized_run_id = run_id or generate_run_id()
        self._write(
            ProcessEvent(
                initialized_run_id,
                ProcessEventType.RUN_INITIALIZED,
                status=RunStatus.STARTING,
                payload={"plan": plan},
            )
        )
        return initialized_run_id

    def emit(self, event: ProcessEvent) -> None:
        self._write(event)

    def heartbeat(self, run_id: str) -> None:
        self._write(ProcessEvent(run_id, ProcessEventType.HEARTBEAT))
