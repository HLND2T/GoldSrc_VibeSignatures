...
- all-gamever 时按 `configs/config.yaml.gamevers` 顺序检查各 `configs/<tag>.yaml`，选首个同时具备 module、支持平台、确切 binary 和 predecessor artifact 的 `REFERENCE_GAMEVER`；两平台均声明时必须来自同一 tag。
