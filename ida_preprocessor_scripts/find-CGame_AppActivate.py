#!/usr/bin/env python3
"""Locate CGame_AppActivate through its activation diagnostics.
...
def split_merged_function(owner, entry, span):
    ...
    if not ida_funcs.set_func_end(owner_start, int(entry)):
        return 'failed to truncate the merged owner'
    if not ida_funcs.add_func(int(entry), int(span['end'])):
        return 'failed to define the recovered entry'
    try:
    ...
    if recovered is None or int(recovered.start_ea) != int(entry) or int(recovered.end_ea) != int(span['end']):
        return 'recovered entry did not become a function start'
    return None
...
async def preprocess_skill(
    ...
    code = LOCATE_PY.replace("ANCHOR_STRINGS_PLACEHOLDER", repr(ANCHOR_STRINGS))
    try:
