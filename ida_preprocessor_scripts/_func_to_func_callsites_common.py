"""Locate direct CALL/JMP sites from an owner function to a callee.

Each qualifying instruction is a separate patch artifact numbered in owner-body
instruction address order, starting at 0:
...
byte displacement from that match to the CALL/JMP. This helper always starts the
...
try:
    ...
    for ea in idautils.FuncItems(int(owner.start_ea)):
        ...
        if not is_direct_rel32_branch(ea):
            continue
        if int(callee.start_ea) not in xref_function_starts(ea):
            continue
...
def expected_callsite_outputs(expected_outputs, patch_name_prefix):
    indexed = {}
...
async def locate_callsites(session, owner_ea, callee_ea):
    code = LOCATE_PY.replace("OWNER_EA_PLACEHOLDER", str(int(owner_ea))).replace(
        "CALLEE_EA_PLACEHOLDER", str(int(callee_ea))
    )
    try:
...
async def preprocess_func_to_func_callsites(
    session,
    skill_name,
    expected_outputs,
    new_binary_dir,
    ...
    *,
    ...
    if platform not in {"windows", "linux"}:
        return False
    expected = expected_callsite_outputs(expected_outputs, patch_name_prefix)
    if expected is None:
        if debug:
            print(f"  {skill_name}: expected outputs are not contiguous callsite indexes")
        return False
    ...
    if inspected_owner_ea != owner_ea:
        return False
    located = await locate_callsites(session, owner_ea, callee_ea)
    ...
    if len(sites) != len(expected):
        if debug:
            print(f"  {skill_name}: found {len(sites)} callsites, expected {len(expected)}: {sites}")
        return False
    for index, (patch_name, output) in enumerate(expected):
        ...
        if unique_ea != patch_ea:
            if debug:
                print(f"  {skill_name}: {patch_name} signature is not unique at {site.get('ea')}: {unique_ea}")
            return False
        if debug:
            print(f"  {skill_name}: {patch_name} ea={site['ea']} {site.get('disasm', '')}")
        write_patch_yaml(
            output,
            {
