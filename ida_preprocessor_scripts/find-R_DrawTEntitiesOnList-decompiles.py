#!/usr/bin/env python3
"""Recover the parse counter and byte stride of the transparent-entity list.

``cl_parsecount`` and ``size_of_frame`` are read while the list is walked to
decide which entities need an interpolated studio frame. ``r_entorigin`` is not
recovered here: the sprite renderer is its producer, because it reads the
...
async def preprocess_skill(
    ...
    return await preprocess_common_skill(
        ...
        generate_yaml_desired_fields=[
            ...
            ("size_of_frame", list(SCALAR_FIELDS)),
        ],
        debug=debug,
    )
