...
Planner 会签入固定证据字段 `cache_mode=warm`，不再由 repository variable 控制 analysis lifecycle。Analysis 在
`win64` Environment 下的专用 `[self-hosted, windows, x64]` runner 运行，并受 repository-wide IDA concurrency
group 约束（动态 per-binary MCP endpoint 为 invocation-scoped，不会放宽该 group）。

所有 analysis 路由都把 producer 从 consumer 中拆出。`plan` 以 `scope: bound-plan` 调用 reusable
...
Unit 与 workflow-contract test 不能替代专用 runner 上记录的首次 miss/publication 与后续 hit run。
...
`preflight` 解析并绑定 `version`、`source_sha` 与 `mode`；`warmup-idb` 始终以 `scope: release-all` 调用 reusable
producer；`build` 是纯 consumer。
...
从 CI 驱动 analyzer 时，传入与本地运行相同的参数——参见[二进制获取与符号分析](analysis.md#分析配置的符号)。
Cache mode 不可配置：每次调用都是 strict warm consumer。对每个配置 tag 的批量分析使用 `-allgamever`；单 tag
运行使用 `-gamever`。CI job 若只需知道二进制是否已就位，使用 `copy_depot_bin.py ... -checkonly`。
