...
- Correct approach: `prompt/call_llm_scoreinfo.md` requests one frags store. `_scoreinfo_dataflow.py` validates the current Windows handler's BEGIN_READ / player-byte / four-short protocol, tracks the first short and the player index through register copies and affine stride arithmetic, meets equal facts at control-flow merges, and rejects cycles, unsupported value flows and nonunique stores. It supplies a current-target instruction rule to the existing LLM retry and live instruction checks. Linux keeps its zero-member named frags rule. No reference address/register is copied and the LLM remains the symbol-mapping step.
