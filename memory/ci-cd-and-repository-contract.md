...
`publish-release` is the only release workflow job with `contents: write`. Manual `source_artifact_mode=tracked` skips full analysis, rebuild comparison, and the whole warm-IDB stage (`warmup-idb` is not scheduled; no selection is downloaded or restored) while retaining runtime evidence and everything downstream; the default remains `rebuild`. A schema-4 tracked manifest therefore carries no `warm_idb_selection_sha256` and no `evidence/cache-selection.json`, because nothing in that pipeline reads an IDB. The trigger skill asks for the build path only when unspecified and passes the selected mode explicitly. See
