...
- SvEngine `Host_LoadBasePalette` loads `palette.lmp` into a callee-saved register, then `test`/`jz` on `COM_LoadHunkFile` before `mov [esp+4], ebp`. Prove the error path terminates using IDA `FlowChart` successors, including non-returning calls; a larger target address alone does not prove it cannot jump back. Remove the candidate definition from the graph and reject it if the use remains reachable from entry. Unknown edges remain potentially reachable.
- Shared argument decoder callers that supply whole functions must include instruction EAs and control-flow successors. `Draw_FillRGBABuf` supplies both; otherwise its initial capacity branches make even local GL arguments unprovable. Regression coverage includes guarded local arguments, an out-of-line block that jumps back past the definition, missing/indirect error edges, and a decoded non-returning call.
- CoF's cached IDB misidentifies the palette file loader as `__exit`, incorrectly cutting its return edge. Do not trust inferred no-return flags to prove dominance: ordinary calls retain fall-through. `host_basepal` declares and revalidates `Sys_Error` as an input. Compiler `__stack_chk_fail` is accepted only through the import table (including ELF symbol versions), not a guessed local name. These verified targets cut control flow, proving the SvEngine error block cannot jump back through the instruction after `Sys_Error` while retaining HL25's stack-protector exit.
- After `Hunk_AllocName`, only proven keep-alive instructions may preserve the return register: `mov` copies, `cmp`/`test`/`push`, and `add`/`sub esp, imm`. A full-width `xor reg, reg` or `lea reg, ...` invalidates only its explicit destination: HL-8684 Linux has `xor edx, edx`, and HL25 Linux has `lea ecx, ...`, before the EAX store. Implicit EAX writes such as `mul` kill the live set.
