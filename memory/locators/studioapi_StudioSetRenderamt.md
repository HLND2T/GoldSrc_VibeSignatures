...
5. Gate the body with `SLOT_SHAPE_WRITE_ALLOW_READS` (`gv_names = ("r_blend",)`): the sole non-derived writable-data store target, reads unrestricted. The accessor reads `currententity` before the call and the x87 builds read `r_blend` back between its two stores. On PIC builds, the collector tracks `lea reg,(r_blend-GOT)[ebx]` through to the actual `fstp [reg]`; the LEA supplies the encodable GV reference. No read-only fallback is allowed. Emit the entry and [[r_blend]] together.
...
- **Do not skip register-state updates for instructions without disp32.** `mov eax,[eax]` must invalidate the tracked address of `currententity`; its runtime pointee is unknown. The shared collector now updates every decoded instruction and recognizes known-global `o_phrase` accesses, preventing the former false `currententity + 0x2FC` store while recovering the real `r_blend` store.
