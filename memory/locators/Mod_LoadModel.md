...
- This artifact is the predecessor of three downstream skills (`find-Mod_LoadModel-decompiles` → `FS_Open`, `find-Mod_LoadModel-brushmodel-decompiles` → `Mod_LoadBrushModel`, `find-Mod_LoadModel_to_FS_Open_callsites`), all of which locate `Mod_LoadModel` again by `func_va` and re-verify that the VA is a function *start*. A wrong (wrapper) root breaks all three at once.
