...
- `cl.players` (gv `cl_players`) = array base recovered from the `cl.players[playerindex].model[0]` byte test in studioapi: a player-indexed byte o_displ/o_mem (or lea) operand whose index register scales to 0x24C (WON hl-3248..3647) or 0x250 (hl-4554+, SvEngine, cof) — never 0x20C (DM_PlayerState) and never the non-player DM_RemapSkin chains (index does not trace to the stack argument). `player_info_t.model` sits at +0x130 in every family (leading userid/userinfo[256]/name[32]/spectator/ping/packet_loss never moved; DWARF-verified on both official hw.so), so `gv_va = access_disp − 0x130`. SvEngine Linux is a PIC two-hop: `lea edx, (X−2EE000h)[ebx]` then `[edx+eax+disp2]` → anchor + D1 + D2 − 0x130. The cl_players gv_inst anchors the model access itself: its disp (+ GOT base) = `&cl.players[0].model` = gv_va + 0x130 — downstream consumers must subtract; the finder re-derives the address on every run. Closures: hl-8684 linux cl_resourcesonhand(0xc44744)−4 + players@0x1a40e0 = 0xde8820 = artifact; hl-10210 linux cl(nMax 0xc2fa80) + players@0x1a58e0 = 0xdd5360 = artifact.
- Regression rows (2026-09-09 second delivery): Host_IsSinglePlayerGame 13/13, cl_players 13/13; finder scripts `find-Host_IsSinglePlayerGame.py` + `find-cl_players.py`, both consuming the studioapi artifact via expected_input.
